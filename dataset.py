...
class Graph_Bert_Dataset_3D(object):
    ...
    def get_data(self):
        ...
        # 划分训练 / 测试
        train_idx = data.sample(frac=0.9, random_state=42).index
        data1 = data.loc[train_idx]
        data2 = data.drop(train_idx)

        # -------- Train Dataset --------
        dataset1 = tf.data.Dataset.from_tensor_slices(
            (data1[self.smiles_field].tolist(),
             data1[self.structure_paths_field].tolist())
        )

        dataset1 = (
            dataset1
            .map(self.tf_numerical_smiles_3d, num_parallel_calls=tf.data.AUTOTUNE)
            .padded_batch(
                64,
                padded_shapes=(
                    [None],
        ...
        dataset2 = tf.data.Dataset.from_tensor_slices(
            (data2[self.smiles_field].tolist(),
             data2[self.structure_paths_field].tolist())
        )
    ...
    def numerical_smiles_3d(self, smiles, structure_path):
        ...
        atoms_list = ['<global>'] + atoms_list
        nums_list = [str2num.get(i, str2num['<unk>']) for i in atoms_list]
        actual_seq_len = len(nums_list)  # 实际原子数（包括<global>）

        # 关键修复：确保坐标数量与原子数量匹配（不包括<global>）
        if len(coords) != actual_seq_len - 1:
            # 如果坐标数量不匹配，进行调整
            if len(coords) > actual_seq_len - 1:
                coords = coords[:actual_seq_len - 1]
            else:
                # 填充缺失的坐标
                padding = np.zeros((actual_seq_len - 1 - len(coords), 3))
        ...
        # 确保 adjoin_matrix 的形状正确
        # adjoin_matrix 应该已经是 (len(atoms_list)-1, len(atoms_list)-1)
        # 我们需要扩展到包括 <global>
        temp = np.ones((actual_seq_len, actual_seq_len))
        temp[1:, 1:] = adjoin_matrix

        # 验证形状
        assert adjoin_matrix.shape == (actual_seq_len - 1, actual_seq_len - 1), \
            f"原始 adjoin_matrix shape mismatch: {adjoin_matrix.shape} vs ({actual_seq_len - 1}, {actual_seq_len - 1})"

        # 现在我们有正确的 adjoin_matrix（包括<global>）
        adjoin_matrix = (1 - temp) * (-1e9)

        # 掩码处理
        choices = np.random.permutation(actual_seq_len - 1)[:max(int((actual_seq_len - 1) * 0.15), 1)] + 1
        y = np.array(nums_list).astype('int64')
        weight = np.zeros(actual_seq_len)
        ...
        # 注意：现在 x, adjoin_matrix, coords, y, weight 都有相同的长度：actual_seq_len

        # 但是，批处理时会进行填充，所以我们需要确保在批处理时 adjoin_matrix 和 coords 也被正确填充
